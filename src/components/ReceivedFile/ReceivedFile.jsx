...
const ReceivedFile = () => {
  ...
  const [user, SetUser] = useState(localStorage.getItem("userId").toString().slice(1, -1));
  ...
  const getReceivedFilesData = async () => {
    try {
      const response = await axios.get(`http://localhost:8080/getDataByEmpId/${user}`);
  ...
  return (
    <>
            ...
            {filesData.map((item, key) => (
              <tr className={styles.tr} key={key}>
                ...
                <td className={styles.td}>
                  <button className={styles.receiveButton} onClick={() => handleReceive(item)}>
                    Receive
                  </button>
                </td>
              </tr>
      ...
      {/* Popup Modal */}
      <Popup className={styles.Popup_component} open={isModalOpen}  closeOnDocumentClick onClose={() => setIsModalOpen(false)}>
            ...
            <table className={styles.modalTable}>
              <tbody>
                ...
                <tr>
                  ...
                  <td>{selectedFile.empName} - {selectedFile.empNumber}</td>
                </tr>
                <tr>
